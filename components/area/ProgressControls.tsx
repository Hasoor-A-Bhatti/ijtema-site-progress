...
export default function ProgressControls({
  ...
  const availableStatuses = getStatusOrder(areaType);

  const displayStatus = availableStatuses.includes(status)
    ? status
    : availableStatuses[0];

  const currentStageIndex = availableStatuses.indexOf(displayStatus);
    ...
    <div>
      <div className="flex items-center justify-between gap-4">
        ...
        <span className="text-sm text-slate-500">
          Stage {currentStage} of {availableStatuses.length}
        </span>
      </div>

      {/* PROGRESS BAR */}
      <div className="mt-3 flex gap-1.5">
        {availableStatuses.map((stage, index) => (
          <div
            key={stage}
      ...
      <div className="mt-6">
        ...
        <div className="grid gap-2">
          {availableStatuses.map((statusKey) => {
            ...
            const active = displayStatus === statusKey;
            ...
            return (
              ...
              >
                ...
                <span className="flex-1">
                  {getStatusLabel(statusKey, areaType)}
                </span>
