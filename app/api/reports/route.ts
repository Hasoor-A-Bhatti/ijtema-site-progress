...
export async function GET(request: NextRequest) {
  ...
  if (!session) {
    return NextResponse.json(
      { error: "Reporting sign-in is required." },
      { status: 401 }
    );
  }
  ...
  if (session.role !== "admin") {
    return NextResponse.json(
      { error: "Admin reporting access is required." },
      { status: 403 }
    );
  }
  ...
  const [departmentsResult, reportsResult] = await Promise.all([
    supabaseServer
  ...
  if (departmentsResult.error) {
    console.error("Report departments:", departmentsResult.error);
  ...
  if (reportsResult.error) {
    console.error("Department reports:", reportsResult.error);
  ...
  const departments =
    (departmentsResult.data ?? []) as ReportDepartmentRow[];
  const reports = (reportsResult.data ?? []) as DepartmentReportRow[];

  const reportMap = new Map(
    reports.map((report) => [report.department_id, report])
  );

  const now = new Date();
  const defaultDeadline = getReportDeadline(requestedDate);

  const departmentStates = departments.map((department) => {
    const report = reportMap.get(department.id) ?? null;
    const deadline = report?.deadline_at
      ? new Date(report.deadline_at)
      : defaultDeadline;
    const status = deriveReportStatus(report, deadline, now);
    const completedFields = countCompletedReportFields(report);
    ...
    return {
      ...
      status,
      completedFields,
      draftCompletedFields: completedFields,
