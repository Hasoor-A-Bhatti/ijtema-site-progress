...
export async function POST(
  ...
  const submissionError = validateReportForSubmission(validated.values);
  ...
  const departmentResult = await supabaseServer
  ...
  if (existingResult.error) {
    console.error("Existing report check failed:", existingResult.error);
  ...
  if (existingResult.data?.submitted_at) {
    // Already submitted: update content, but preserve the very first submitted_at.
    ...
    if (updateResult.error || !updateResult.data) {
      console.error("Submitted report update failed:", updateResult.error);
    ...
    const report = updateResult.data as DepartmentReportRow;
  ...
  if (existingResult.data) {
    ...
    if (updateResult.error) {
      console.error("Report submit failed:", updateResult.error);
    ...
    if (!updateResult.data) {
      const reread = await supabaseServer
        .from("department_reports")
        .select("*")
        .eq("id", existingResult.data.id)
        .single();
      ...
      if (reread.error || !reread.data) {
        return NextResponse.json(
          { error: "Report submission could not be confirmed." },
          { status: 500 }
        );
      }

      const report = reread.data as DepartmentReportRow;
  ...
  if (insertResult.error || !insertResult.data) {
    console.error("Report insert/submit failed:", insertResult.error);
  ...
  const report = insertResult.data as DepartmentReportRow;
